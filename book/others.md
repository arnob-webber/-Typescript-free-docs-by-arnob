...
type Id = `${Department}-${Language}-id`; 
```
...
makeGreeting('Simon');
makeGreeting(['Simone', 'Sydney Sweeney']);
```
...
type A = CheckNumber<123>; 
type B = CheckNumber<'abc'>; 
```
...
```typescript
type AnimalCallback<out T> = () => T; 
```

And for Contravariant, use the `in` keyword:

```typescript
type AnimalCallback<in T> = (value: T) => void; 
```
...
user.attributes?.map(console.log); 
user.nickName; 
...
user2.attributes?.map(console.log); 
user2.nickName; 
...
user3.attributes?.map(console.log); 
user3.nickName; 
```
